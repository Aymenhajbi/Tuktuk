...
describe('OrchestratorService', () => {
  ...
  it('creates decision in transaction path', async () => {
    ...
    repository.inTransaction.mockImplementation(async (fn: any) => fn({}));
  ...
  it('throws on invalid confidence domain rule', async () => {
    ...
    await expect(service.processTrendSignal(dto)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('maps transaction failure', async () => {
    ...
    await expect(service.processTrendSignal(dto)).rejects.toBeInstanceOf(InternalServerErrorException);
  });
  ...
  it('throws conflict when idempotency in progress', async () => {
    ...
    await expect(service.processTrendSignal(dto)).rejects.toBeInstanceOf(ConflictException);
  });
});
